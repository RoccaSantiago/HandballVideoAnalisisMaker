...
def main_cut(download_path, array_clips, direction):
    ...
    while True:
        ...
        if option == '1':
            ...
            start_time = input("\nInserte en qué minuto y segundos arranca el clip separado por un espacio (ej. 1 24): \n\n").split()
            start_time = int(start_time[0]) * 60 + int(start_time[1])

            end_time = input("\nInserte en qué minuto y segundos termina el clip separado por un espacio (ej. 1 24): \n\n").split()
